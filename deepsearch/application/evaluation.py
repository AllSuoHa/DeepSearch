...
class ResearchQualityEvaluator:
    ...
    def evaluate(
        ...
        usable = sum(bool(source.usable_text) for source in sources)
        domains = {self._domain(source.url) for source in sources}
        source_quality = round(sum(source.quality_score for source in sources) / total * 100)
        grounding = 100 if validation.valid else max(0, 100 - 18 * len(validation.issues))
        coverage = 100 if validation.valid else min(90, round(30 + 16 * len(plan.subquestions) + (10 if trace_count >= plan.minimum_rounds else 0))) if sources else 0
        diversity_target = max(2, min(5, len(sources)))
        diversity = min(100, round(len(domains) / diversity_target * 100)) if sources else 0
        retrieval = round(usable / total * 100) if sources else 0
        ...
        dimensions = (
            ...
            QualityDimension("来源多样性", diversity, f"{len(sources)} 个来源来自 {len(domains)} 个独立域名。"),
            QualityDimension("来源质量", source_quality, "综合权威域名、正文完整度、可抓取性与内容相关性。"),
            QualityDimension("检索健康度", retrieval, f"{usable}/{len(sources)} 个来源具有可用正文或摘要。"),
        )
