...
问答模式完全不发起网页搜索，适合配合本机 Ollama 控制日常成本。无论问题中是否包含天气、新闻、比较或研究等词，系统都不会在问答内部重新路由，也不会回复“请切换搜索/研究”；对于无法联网核实的内容，模型会说明知识时效或不确定性，并继续在当前能力范围内回答。每条最新问答结果下仍提供“改用搜索重跑”和“改用研究重跑”，是否切换完全由用户决定。问答会保存在会话中，但不会生成搜索快照、研究报告或资料库资产。
...
“设置 → 搜索 → 默认搜索内容”内置“综合、新闻、知识、公告、学术”。在“自定义信息类型”中输入名称并按 Enter 可新增，移除标签后保存即可删除；保存后的类型也会同步出现在主页高级选项中。“综合”保持原始查询；其余选项会给查询加入对应的聚焦词，其中“学术”还会启用 OpenAlex 和 Crossref。这个选项只调整检索方向，不调用大模型。

“设置 → 搜索 → 搜索源状态”展示当前可用的**检索后端**，与大模型配置无关：

- Brave Search 是独立的网页搜索 API；配置 `DEEPSEARCH_BRAVE_API_KEY` 后作为主搜索源。需先在 [Brave Search API 控制台](https://api-dashboard.search.brave.com/) 启用套餐并创建密钥，再将密钥写入 `.streamlit/secrets.toml` 或同名环境变量；
- DuckDuckGo 和 Wikipedia 是无需项目密钥的备用来源；
...
输入区是一张单层白色卡片，文本区没有额外内框。左下角先显示带胶囊底纹的问答、搜索和研究选择器，右侧紧跟高级选项；右下角是模型设置与圆形主操作。圆形按钮空闲时为居中的发送箭头，提交后变为停止图标；再次点击会请求停止，随后同一位置变为继续图标。任务成功或失败后会立即恢复空闲提交状态。停止仍是协作式取消：系统会在下一个阶段边界中止后台任务，但若程序正阻塞在一次 HTTP 调用中，需要等该调用返回或超时后才能观察到停止信号。未完成内容不会保存为正式资产。“继续执行”从原问题重新运行并复用已有用户消息，不会重复添加相同问题。模型 HTTP 请求不支持从已中断的 token 位置断点续传，因此这里的“继续”是重新开始本轮任务。
