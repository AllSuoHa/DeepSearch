...
- `ChatService` 返回短消息并优先调用有效 `chat_llm`；本地时间与计算只作模型不可用时的降级；
...
快速回答模型和研究模型默认保留两套配置，也允许用户显式开启“使用相同配置”。共享的是 OpenAI 兼容连接参数，不是执行流程：CHAT 仍只调用 `ChatService`，RESEARCH 仍只调用研究报告器。问答系统提示声明无实时 Tool，模型失败也不自动调用搜索或研究。

### 2.2 显式上下文与 Provider Registry

`ContextPolicy` 把 `fresh`、`conversation`、`follow_up` 带入统一请求，页面不再只因存在 `last_result` 就调用追问。模式重跑和重新执行强制 fresh；普通研究输入由 `ContextResolver` 识别明确指代，并用当前 `conversation_id` 隔离上一结果。

搜索实现保留统一 `SearchProvider`，新增能力标签和 `SearchProviderRegistry`。Tavily、Brave、SearXNG、DuckDuckGo 提供通用/时效能力，Wikipedia 只提供背景能力，OpenAlex/Crossref 只提供学术能力。排序先验证相关性，再应用权威性加分；无关候选在抓取或报告前移除。适配器失败会有限重试，服务实例内连续失败会熔断，同时保留降级原因。

### 2.3 轻量 Clean Architecture
...
维护语义：Tool 是实际代码能力，Function Calling 只是模型请求 Tool 的协议，Skill 是操作说明，SearchProvider 是受审查的数据源适配器。本版不为“看起来像 Agent”开放自由工具调用；未来若给问答模型增加 Function Calling，必须先实现白名单、参数校验、轮数/超时/输出上限、URL 安全和 tool message 回传。
...
首页输入区使用一个状态化主操作：`idle` 显示“发送”，`running` 显示可停止的“暂停生成”，`resumable` 同时允许“继续执行”和发送编辑稿。运行中的最后一条用户消息显示复制与修改入口；修改和暂停共用取消路径。暂停后输入框不再被 `resumable` 状态锁定，原问题自动回填；编辑后通过受限的 `replace_last_user_message()` 只改写末尾尚无助手回答的消息。错误卡片的重试先把对应消息持久标记为 `retried`，再通过回调排队，因此旧入口在提交当轮立即消失且不会复制原用户消息。协作式取消通过 tracker 的线程事件传到下一阶段回调；阻塞中的 HTTP 调用仍由各自超时控制。
...
| `test_chat_mode.py` | 19 | 显式模式、旧 AUTO 路由、模型优先、本地降级、上下文和配置安全 |
| `test_context_semantics.py` | 7 | fresh、conversation、follow_up、重跑和会话隔离 |
| `test_conversations.py` | 15 | 会话、逐条来源恢复、暂停编辑、快照、快捷输入、回收站和路径边界 |
...
| `test_search_providers.py` | 6 | Provider Registry、Tavily、SearXNG、降级、相关性和密钥边界 |
| `test_storage.py` | 2 | 多格式存储与资料库安全重命名 |
| `test_streamlit_app.py` | 21 | 页面、问答资产隔离、暂停编辑、控件、后台状态和视觉合同 |
| `test_topics.py` | 6 | 任务迁移、周期、幂等和失败隔离 |
| `test_verification.py` | 7 | 引用、支持度、重复、来源重建、Markdown 规范化和噪声 |

总计 145 项，另有参数化子测试。真实网络和真实模型不进入默认回归，因为内容、频率限制和可用性不稳定。可以增加非阻断契约测试，但不能替代离线单元测试。
...
1. 先检查许可证、安全性、数据流、依赖规模和维护状态；不得直接运行来源不明的 GitHub Skill。
2. 实现 `SearchProvider.name`、`capabilities`、状态字段和 `search()`。
3. 映射为统一 `SearchResult`，保存 provider、query、URL、发布时间和检索时间，只返回合法 URL。
4. 在 `build_search_providers()` 注册并声明适用路由。
5. 明确超时、有限重试、限流、异常和地区/语言上下文。
6. 增加解析、路由、单源失败和无关结果过滤回归。
