...
class ChatModeTests(unittest.TestCase):
    ...
    def test_current_time_and_calculation_are_only_no_model_fallbacks(self):
        fixed = datetime(2026, 9, 9, 1, 2, 3, tzinfo=timezone.utc)
        service = ChatService(now_provider=lambda zone: fixed)
        ...
        model = RecordingChatModel("由模型回答")
        modeled = ChatService(model, now_provider=lambda zone: fixed).reply("现在几点")
        self.assertEqual(modeled.answer, "由模型回答")
    ...
    def test_explicit_chat_answers_in_place_without_mode_guidance(self):
        ...
        self.assertTrue(all("没有实时搜索、网页访问" in system for system, _ in chat_model.calls))
        self.assertTrue(all("不得编造当前数据" in system for system, _ in chat_model.calls))
    ...
    def test_explicit_chat_realtime_question_never_searches(self):
        ...
        with tempfile.TemporaryDirectory() as directory:
            ...
            with patch(
                ...
                return_value="我没有实时工具，无法确认今天的天气。",
            ) as generate:
                run = DeepSearchAgent(settings, search_providers=[provider]).run(
        ...
        self.assertEqual(run.resolved_mode, WorkMode.CHAT)
        self.assertIn("没有实时工具", run.content)
        generate.assert_called_once()
        self.assertEqual(provider.calls, 0)

    def test_realtime_question_calls_chat_model_and_receives_capability_prompt(self):
        model = RecordingChatModel("无法联网确认，请使用搜索重跑。")

        result = ChatService(model).reply("上海今天的天气怎么样")

        self.assertEqual(len(model.calls), 1)
        self.assertIn("没有实时搜索、网页访问", model.calls[0][0])
        self.assertIn("无法联网确认", result.answer)
    ...
    def test_chat_can_explicitly_share_research_connection_without_changing_mode(self):
        settings = Settings(
            runtime_mode="online",
            llm=LLMSettings(
            ...
            chat_uses_research_model=True,
        )

        self.assertTrue(settings.chat_model_available)
        self.assertEqual(settings.effective_chat_llm.model, "research-model")
        with patch(
            "deepsearch.infrastructure.llm.OpenAICompatibleLLM.generate_chat",
            return_value="共享连接的问答回复",
        ) as generate:
            run = DeepSearchAgent(settings, search_providers=[CountingSearch()]).run(
                AgentRequest("解释闭包", WorkMode.CHAT)
            )
        generate.assert_called_once()
        self.assertEqual(run.resolved_mode, WorkMode.CHAT)
        self.assertEqual(run.audit.model_name, "research-model")
        self.assertFalse(run.audit.used_search)
